...
public class ListViewColumnSorter : IComparer
{
    ...
    public int Compare(object x, object y)
    {
        ...
        string s1 = (itemX.SubItems.Count > _columnToSort)
            ? itemX.SubItems[_columnToSort].Text
            : string.Empty;

        string s2 = (itemY.SubItems.Count > _columnToSort)
            ? itemY.SubItems[_columnToSort].Text
            : string.Empty;

        int result = _comparer.Compare(s1, s2);

        if (_order == SortOrder.Ascending) return result;
        if (_order == SortOrder.Descending) return -result;
        return 0;
    }
